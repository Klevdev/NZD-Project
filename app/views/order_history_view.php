<div class="trains-main--block">
    ...
    <div class="table--block">
        <div class="table-header--block">
            ...
            <div>Откуда - Куда</div>
            <div>Время отправки поезда</div>
            <div></div>
        </div>
        <?php
        if ($data['count'] === false)
            echo '<div class="table-row--block"><div style="text-align: left; text-align: center; width: 100%; padding: 15px 50px;">Заказы отсутсвуют</div></div>';
        else
            foreach ($data['orders'] as $value)
                ...
                <div class="table-row--block">
                    <div>' . $value['id'] . '</div>
                    <div>' . $value['name'] . '</div>
                    <div>' . date('d.m.Y H:i:s', strtotime($value['start_time'])) . '</div>
                    <a href="/user/orders?id=' . $value['id'] . '">Просмотреть места</a>
                </div>';
        ?>
    </div>
</div>
